...
import random
import joblib # For XGBoost models
import winsound
...
from EEG_generate_training_matrix_realtime import gen_training_matrix # For real-time data preprocessing

''' May not be necessary as of now, but filters raw signal values for precise measure '''
...
def main():
    ...
    # replace with improved model in the future    
    ...
    try:
        ...
        while True:
            ...
            # generates a matrix(input for the model) every 5 seconds
            matrix = gen_training_matrix(np_data, [])

            """ 1. Make a raw prediction """
            ...
            """ 2. Interpret raw result into readable format """
            pred_arr = np.array(list(map(lambda x: np.argmax(x), raw_pred)))

            print('\npred_arr{}: {}'.format(pred_arr.shape, pred_arr))
            confids = []
            ...
            ''' update 3rd elif after the improved model that contains the fourth state "Stationary" '''
            if state_num == 0:
                ...
                # commands.building()
            # elif state_num == 3:
            #     print('Current state: Stationary')
            #     commands.stationary()
            else:
            ...
            ''' To determine if the program finished executing '''
